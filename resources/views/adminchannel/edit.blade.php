...
<!-- Default box -->
  <div class="box">
    ...
    <div class="box-body">
      ...
      <form role="form" method="post" class="form-horizontal" action="{{ url('admin-channel/edit/5afefdddba62fc0e300e2952') }}" enctype="multipart/form-data">
        @csrf
        <div class="box-body">
          <div class="form-group">
            <label for="channelName" class="col-sm-2 control-label">Name</label>
            <div class="col-sm-10">
            <input type="text" class="form-control" name="channelName" id="channelName" value="{{$channels->channelName}}" placeholder="Enter channel name" maxlength="30">
           </div>
          </div>
          ...
          <div class="form-group">
            ...
            <div class="col-sm-10">
            <input type="text" class="form-control" name="shortname" id="shortname" value="{{$channels->shortname}}" placeholder="Enter shortname" maxlength="10">
           </div>
          </div>
          ...
          @if(!empty($channels->api))
            @foreach($channels->api as $key => $value)
              <span class="c-api" id="Wrapper_{{ $key }}">
                <div class="form-group">
                  <div class="col-sm-5">
                    <select class="form-control" id="apiType_{{ $key }}" name="apiType[]" required="">
                      <option value="">select</option>
                      @foreach(config('constants.API_TYPE') as $kk => $val)
                          @if($channels->apiType == $kk || $value['name'] == $kk)
                              <option selected="selected" value="{{$kk}}">{{$val}}</option>
                          @else
                  ...
                  <div class="col-sm-5">
                  <input type="text" class="form-control" name="apiName[]" value="{{$value['api']}}" id="apiName_{{ $key }}" placeholder="Enter api name" >
                 </div>
                  @if($key > 0)
                  <div class="col-sm-2"><a class="btn btn-danger channel_remove"> <i class="fa fa-minus"></i></a></div>
                  @endif
                </div>
              </span>
            @endforeach
